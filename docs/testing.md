...
## Observability tests (Phase 24)

Run with the rest — `npm run test:unit`, `npm run test:integration`, `npm run test:e2e` — and in CI. Rationale and the model: `docs/observability.md`.

| File | Layer | What it pins |
|---|---|---|
| `tests/unit/observability-redact.test.mjs` | unit | what may leave a request: addresses, JWTs, bearer credentials, `key=value` secrets, long tokens and phone-like numbers scrubbed (ids kept); private-link tokens in paths; errors from `Error`, Supabase-style objects, strings, cycles, causes; private field names; depth/width/array/cycle bounds; the fingerprint is the same for the same failure with different ids and numbers and differs by event, message and route |
| `tests/unit/observability-core.test.mjs` | unit | the request id (UUID only, never trusted from a header, concurrent runs do not mix, `bindRequestId` for a Server Action's own frame); the logger (shape, invalid names, truncation, a broken sink never throws); CSP report parsing in both wire formats, extension noise dropped, hostile directives; client error reports |
| `tests/unit/observability-alerts.test.mjs` | unit | every alert rule at its threshold and one step either side, ordering, "never recorded" is not an alert, every rule has e-mail text and a title and an action in **both** languages |
| `tests/unit/observability-static.test.mjs` | unit (reads the source) | no `console` in server code; every event name well formed; where `getRequestId()` (which calls `headers()`) may be used; the logger, the database client and the store never call `headers()`; the proxy makes its own id; the two open endpoints are rate-limited, size-capped, answer 204 and refuse GET; client/CSP groups never alarm; migration `0021` ↔ code agree (RLS, grants, search paths, sources, rollback lines) |
| `tests/integration/observability.integration.test.mjs` | integration (real PostgreSQL + PostgREST) | the two recording functions (grouping, 30-way race, windows, regression reopens, the 500-group cap, refusal of bad input, truncation), who may call them (not `anon`), `captureError`/`track` (grouping, **nothing private stored** — an address, a name, a token and free text are planted and every row is searched), never throwing, `onRequestError`, a real brief leaving correlated records (inquiry, timeline, e-mails, event, reference lookup), a **simulated database failure while saving a brief** now filed as an error, the audit trail and the failing-open limiter observed, **every alert rule against real rows**, once-a-day e-mails, an alert e-mail that fails not alerting about itself, the owner-only status action, the health check on broken and degraded databases |
| `tests/integration/observability-routes.integration.test.mjs` | integration | the daily run (secret, heartbeat with the id it returns, a failing step does not stop it), the external check (auth, named monitors only, failing/degraded, `raise=0`, never all-clear when it cannot look), the health endpoints (no detail, 15-second reuse), both reporting endpoints under hostile input (oversized, lying `Content-Length`, junk, extension noise, rate limit, never alarming) |
| `tests/e2e/observability.spec.ts` | browser (built app) | the request id and the report endpoint in the policy on every route type; a forged id is not echoed; the health endpoints; the Observability page in both languages with seeded data and no missing translation; resolve/reopen; the reference lookup (digest, id, hostile input, the plain GET form); the editor cannot reach it; **axe on the page in both languages and both themes**; a notification links to its request; **a real blocked inline script in the admin is reported and lands as a CSP group; a real uncaught error in a page is reported once and scrubbed** |

**Known gaps:** no test makes the built app throw a real render error to watch Next.js call `onRequestError` (the hook is tested with the documented argument shapes); the scheduled GitHub workflow is not run by any test — its shell logic is exercised by hand against recorded responses (`PHASE-24-README.md`).
