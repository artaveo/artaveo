...
const featuredProjectsData: Project[] = [
  {
    ...
    context: tx(
      ...
      'این سامانه برای رزرو و مدیریت عملیات شرکت‌های اتوبوس‌رانی بین‌شهری در افغانستان ساخته شده است. این بازار هنوز عمدتاً با گیشه‌های فروش دستی و رزرو تلفنی اداره می‌شود و سامانه‌ی مشترکی برای اتصال فروش بلیت به عملیات ناوگان و سفرها وجود ندارد. مسافران سفر را جست‌وجو و صندلی را روی نقشه‌ی زنده انتخاب می‌کنند. اپراتورها هم مسیرها، ناوگان، رانندگان و سفرها را از یک پنل متصل به پایگاه داده مدیریت می‌کنند.',
    ),
    problemAndGoals: tx(
      ...
      'وضعیت صندلی و پرداخت داده‌های حیاتی این سامانه‌اند و نباید مرورگر درباره‌ی آن‌ها تصمیم بگیرد. اگر دو نفر یک صندلی را هم‌زمان بگیرند یا پرداخت یک رزرو لغوشده همچنان دریافت‌شده بماند، سیستم قابل‌اعتماد نیست. برای همین، تصمیم‌های حساس رزرو، تأیید و پرداخت در پایگاه داده و سمت سرور اجرا می‌شوند.',
    ),
    constraints: [
      tx(
  ...
  },
  {
    ...
    quality: tx(
      ...
      'استراتژی cache با یک build واقعی و preview بررسی شد، نه فقط در حالت توسعه. service worker در توسعه غیرفعال است. دسترسی Department Admin نیز fail-closed است: پیش‌فرض رد و فقط با اجازه‌ی صریح مجاز. تست خودکار اختصاصی و CI/CD وجود ندارد و استقرار با build محلی و آپلود انجام می‌شود.',
    ),
    currentStatusAndNext: tx(
