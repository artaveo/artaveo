...
## Phase 9 — Start a Project (Inquiry v1 — first real backend slice)  ⏳ PARTIAL (9.1 ✅ complete — see `docs/phases/PHASE-9.1-README.md`; 9.2 ⏳ PARTIAL — code complete, database provisioned & migration applied, awaiting env-var configuration + end-to-end verification, see `docs/phases/PHASE-9.2-README.md`; 9.3 not started, blocked on D-01)
...
### 9.2 Server handling & persistence (minimal slice of the Phase 12 schema)  ⏳ PARTIAL — DB live, awaiting env-var configuration + end-to-end verification (see `docs/phases/PHASE-9.2-README.md`)
...
**Status (13 September 2026):** D-10 resolved — see the Decision Register. The `Artaveo` Supabase project is live (`ukzovqnpqjcrwofycalc`, `ap-southeast-1`, free plan) and `db/migrations/0001_inquiries.sql` has been applied: `inquiries`/`inquiry_events` exist with RLS enabled and zero policies, confirmed via `Supabase:get_advisors` (only the expected informational `rls_enabled_no_policy` lint, which is the intended design — service-role only, no public access of any kind). All application code (`lib/supabase/server.ts`, `app/actions/inquiries.ts`, the wired Brief Builder) was already complete from the prior session. **What's left, and why it isn't a blocker:** the three env vars (`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `INQUIRY_IP_HASH_SECRET`) are deployment configuration the owner sets directly (in `.env.local` for local dev, and in Vercel's project settings for production) — the agent deliberately never handles the service-role key itself. Once set, an actual end-to-end submission (and the idempotency/rate-limit/honeypot paths) still needs to be exercised for real before this sub-phase can be marked complete — see `docs/phases/PHASE-9.2-README.md`.
